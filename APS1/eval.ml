...
and sem_def def env mem = 
  ...
  | ASTProc(id,args,blk) -> ((id, InP(blk, sem_args args, env ))::env, mem)

  | ASTProcRec(id, args, blk) -> ((id, InPR(blk, id, sem_args args, env))::env, mem)
...
and sem_stat stat env mem flux=
  ...
  | ASTCall(id,expr) -> 
    ...
    let args2 = List.map (fun e -> sem_expr e env mem) expr in
      match v with
