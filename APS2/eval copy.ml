...
and sem_expr expr env mem= 
  ...
  | ASTIf (cond,cons,alt) -> 
      (match (sem_expr  cond env mem) with
      (InZ 1,mem') -> sem_expr cons env mem'
      |(InZ 0,mem') -> sem_expr alt env mem'
      |_ -> failwith "semantique if error") 

  (* | ASTNot b -> if (sem_expr b env) = InZ(1) then InZ(0) else InZ(1) *)
  | ASTAnd (b1,b2) ->(
        let (val1,mem') = sem_expr b1 env mem in
        let (val2,mem') = sem_expr b2 env mem' in

    match ((sem_InZ val1),(sem_InZ val2)) with
          (1,1) -> (InZ 1,mem')
          |(_,_) -> (InZ 0,mem')
    )
  | ASTOr (b1,b2) -> (
  ...
  | ASTApp (exprApp,exprs) ->
    (
      ...
      match e with
      | InA a , mem-> failwith "TODO_APPINA"
      | Address a , mem -> failwith "TODO_APP_ADDRESS"
      | InZ n ,mem -> failwith "App usage : ( expr exprs ) "
      | InF (expr_f,args_f,env_f), mem -> failwith "TODO_APP_INF"
          (* let env_f_prim = mis_a_jour args_f es env_f in
            sem_expr expr_f env_f_prim mem *)
      | _ -> failwith "TODO"
      (* | InFR (expr_f,name_f,args_f,env_f) -> failwith "TODO_APP_INF"
  ...
  | ASTLen(e) -> failwith "TODO"
  | ASTAlloc(e) -> failwith "TODO"
  | ASTNth(e1,e2) -> failwith "TODO"
  | ASTVSet(e1,e2,e3) -> failwith "TODO"
...
and sem_stat stat env mem flux=
  ...
  | ASTSet(id,expr)->( let a = sem_lvalue id env mem and (v,mem') = (sem_expr e env mem) in 
    match a with
...
and sem_lvalue lval env mem =
  match lval with
  | LvalId x ->  
    (match get_value x env with
      |Address a -> a
      |Block (a,n) -> (a+1)
      |_ -> failwith "Not an Lvalue"
)
  | ASTLval(lval,e) -> 
    (match sem_lvalue (env,mem) lval with
      a -> (match sem_expr (env,mem) e with
